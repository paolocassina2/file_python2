...
#################################################
	...
	def callback(event):
			...
			if (event.x >0 and event.x<1000) and (event.y>0 and event.y<1000):
				# ~ canvas1.var.create_line(610, 100, 600, 700)
				canvas1.var.delete("all")
				...
				if die==0:
					testo="head"
					colore="blue"
					listacoin.append("head")
					
				else:
					testo="tail"
					colore="red"
					listacoin.append("tail")
				canvas1.var.create_oval(200+200,300,400+200,500,fill=colore)
				label6.var = tk.Label(root, text=testo)
				label6.var.config(font=('helvetica', 14))
				canvas1.var.create_window(500, 400, window=label6.var)
				
				label8.var = tk.Label(root, text=listacoin)
				label8.var.config(font=('helvetica', 14))
				canvas1.var.create_window(200, 200, window=label8.var)
				# ~ print("ciao")
			####################################################################################	
		
				label9.var = tk.Label(root, text="n_throws "+str(n_throws.var))
				label9.var.config(font=('helvetica', 14))
				canvas1.var.create_window(100, 100, window=label9.var)
