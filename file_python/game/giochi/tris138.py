...
def traccia_riga():
		...
		if caso.var==0:
			cancella_linee()	
				...
				# ~ condizioni_vittoria()
		if vinci.var=="a" or vinci2.var=="a":
...
def entrybox():
	...
	# ~ label6.var = tk.Label(root, text="")
	# ~ label6.var.config(font=('helvetica', 14))
	# ~ canvas1.var.create_window(1000, 130, window=label6.var)
	# ~ label6.var.destroy()
	# ~ caso.var=-10		
	...
	def destroy():
		...
		canvas1.var.create_window(1000, 200, window=label6)
		label6.destroy()
	# ~ destroy()	
	# ~ def contaclick():
		# ~ contaclick.var=0
	...
	label6 = tk.Label(root, text="tris_inizia  PC")
	# ~ label6.var = tk.Label(root, text="")
	label6.config(font=('helvetica', 14))
	canvas1.var.create_window(500, 20, window=label6)				
	# ~ label6.var.destroy()
	...
	# ~ label6.var.destroy()
		# ~ if vinci2.var=="a":
			# ~ label6.var = tk.Label(root, text="vittoria pc")
		# ~ elif vinci.var=="a":
						# ~ label6.var = tk.Label(root, text="")
						# ~ label6.var.config(font=('helvetica', 14))
						# ~ canvas1.var.create_window(500, 50, window=label6.var)
...
def ciao():			
			...
			# ~ elif conta.var%2==0:	
				...
				if vinci.var=="b" and vinci2.var=="b":
						...
						# ~ condizioni_vittoria()
						# ~ label8.var.destroy()
						# ~ if scelta_pc.var in listascelte_possibili.var:	
		...
		# ~ print(2conta.var)
						...
						# ~ label6 = tk.Label(root, text="")
						# ~ label6.config(font=('helvetica', 14))
						# ~ canvas1.var.create_window(1000, 130, window=label6)		
						# ~ label6.destroy()
		# ~ condizioni_vittoria()
...
# ~ def ciao():
					...
					# ~ label6.var = tk.Label(root, text="vince PC")
					# ~ label6.var.config(font=('helvetica', 14))
					# ~ canvas1.var.create_window(10labe00, 200, window=label6.var)
	...
	# ~ button2 = tk.Button(text='dt22222', command=destroy, bg='brown', fg='white', font=('helvetica', 9, 'bold'))
	# ~ canvas1.var.create_window(1000,50, window=button2)		
