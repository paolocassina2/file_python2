...
def condizioni_vittoria():
	...
	lista_vittoria=[]
	for zy in colonna:
		lista_vittoria.append(set(zy))	
	for x in riga:
		lista_vittoria.append(set(x))
	d1=[a[0][0],a[1][1],a[2][2]]
	d2=[a[2][0],a[1][1],a[0][2]]
	lista_vittoria.append(set(d1))
	lista_vittoria.append(set(d2))
		# ~ r2.append(a[x][j])
		# ~ r3.append(a[x][j])
	# ~ lista_vittoria=[set(r1),set(r2),set(r3),set(c1),set(c2),set(c3),set(d1),set(d2)]
	# ~ caso=0
	for xj in lista_vittoria:	#faccio scorrere la lista vittoria , se trovo un set di lunghezza uno significa che uno dei due giocatori ha vinto e il ciclo si interrompe.
		caso.var+=1					#scorrendo la lista capisco anche  di quale caso di vittoria si tratta
		if "-" not in xj:		
			
			if len(xj)==1:
				if "*" in xj:
					vinci.var="a"
					print("caso.var",caso.var," vittoria g1")
					print("xj",xj)
					break
				else:
					vinci2.var="a"
					print("caso.var",caso.var,"vittoria g2")
					break
...
def entrybox():
			...
			if vinci.var=="b" and vinci2.var=="b":	
				if conta.var%2!=0:
					...
					###########################################################################################
						...
						if vinci2.var=="b":
							posizione.var=8
					# ~ print("dfkkld",posizione.var)
					if posizione.var in listascelte_possibili.var:
						# ~ if vinci.var=="b":
							res.var[posizione.var]="*"
							print("aaaaaaaaaaaaaaaaaaaaaaa",posizione.var)
							listascelte_possibili.var.remove(posizione.var)
							print(listascelte_possibili.var)
							print(listascelte_possibili.var)
							disegna_rettangolo()
							conta.var=conta.var+1
							print(conta.var," turno")
							condizioni_vittoria()
				if conta.var%2==0:	
					if vinci.var=="b" and vinci2.var=="b":
						scelta_pc.var=random.choice(listascelte_possibili.var)
	...
	# ~ de		
			...
			# ~ traccia_riga()
			if conta.var==9 and vinci.var=="b" and vinci2.var=="b":
					print("pareggio")
					label6.var = tk.Label(root, text="pareggio")
					label6.var.config(font=('helvetica', 14))
					canvas1.var.create_window(1000, 200, window=label6.var)
			if vinci2.var=="a" or vinci.var=="a":
				traccia_riga()
				if vinci.var=="a":
					label6.var.destroy()
					...
					canvas1.var.create_window(1000, 200, window=label6.var)
# ~ def ciao():	
	label3.var = tk.Label(root, text="TRIS-INIZIA PC")
	label3.var.config(font=('helvetica', 14))
	canvas1.var.create_window(500, 50, window=label3.var)
		# ~ print(res.var)
		# ~ condizioni_vittoria()
		# ~ conta.var=conta.var+1
		# ~ def listascelte_possibili():
		...
		# ~ print("ciao",listascelte_possibili.var)
