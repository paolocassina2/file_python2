...
parola_prima=random.choice(contents).strip()
# ~ print(parola_prima)
# ~ incastro=random.choice(list(parola_prima))
lista=[parola_prima]
for x in range(1,11):
	incastro=random.choice(lista[len(lista)-1])
	while True:
		x3=random.choice(contents)		#la parola scelta deve avere una lettera in comune con l'ultima parola
	# ~ for x3 in contents:
		parola22=x3.strip()
		if len(parola22)>2:
			if incastro in parola22:
				lista.append(parola22)
				break
	...
	# ~ if x >1:
		# ~ print(len(lista),"lenlista")
	# ~ print(lista[len(lista)-1])
		# ~ incastro=random.choice(lista[len(lista)-1])	
		# ~ for x1 in contents:
			# ~ parola_1=x1.strip()
			# ~ if x ==1:
			# ~ if incastro in parola_1:
				# ~ print(parola_1)
				# ~ lista.append(parola_1)
				# ~ break	
	...
	# ~ for x1 in contents:
			...
			# ~ if x ==1:
				# ~ if incastro in parola_1:
					...
					# ~ lista.append(parola_1)
					# ~ break
# ~ print(incastro)	#scegli una lettera a caso dell'ultima parola inserita nella lista
				# ~ print(incastro)	
			# ~ else:
				# ~ incastro=random.choice(lista[len(lista)])	#scegli una lettera a caso dell'ultima parola inserita nella lista
				# ~ print(incastro)
# ~ for j in range(1,4):
	# ~ for x in contents:
		# ~ parola_1=x.strip()
