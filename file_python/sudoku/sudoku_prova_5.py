...
while True:			
	...
	for i,i1,i2,i3,i4,i5,i6,i7,i8,q1,q2,q3 in zip(range(len(primacolonna)),range(len(secondacolonna)),range(len(terzacolonna)),range(len(quartacolonna)),range(len(quintacolonna)),range(len(sestacolonna)),range(len(settimacolonna)),range(len(ottavacolonna)),range(len(nonacolonna)),range(len(primoquadrato)),range(len(secondoquadrato)),range(len(terzoquadrato))):
		for checki,checki1,checki2,checki3,checki4,checki5,checki6,checki7,checki8,checkq1,checkq2,checkq3 in zip(range(len(primacolonna)),range(len(secondacolonna)),range(len(terzacolonna)),range(len(quartacolonna)),range(len(quintacolonna)),range(len(sestacolonna)),range(len(settimacolonna)),range(len(ottavacolonna)),range(len(nonacolonna)),range(len(primoquadrato)),range(len(secondoquadrato)),range(len(terzoquadrato))):
			if i != checki:
				if primacolonna[i] == primacolonna[checki]:
					flag = 1
			if i1 != checki1:
				if secondacolonna[i1] == secondacolonna[checki1]:
					flag1 = 1
			if i2 != checki2:
				if terzacolonna[i2] == terzacolonna[checki2]:
					flag2 = 1
			if i3 != checki3:
				if quartacolonna[i3] == quartacolonna[checki3]:
					flag3 = 1
			if i4 != checki4:
				if quintacolonna[i4] == quintacolonna[checki4]:
					flag4 = 1
			if i5 != checki5:
				if sestacolonna[i5] == sestacolonna[checki5]:
					flag5 = 1
					
			if i6 != checki6:
				if settimacolonna[i6] == settimacolonna[checki6]:
					flag6 = 1
					
			if i7 != checki7:
				if ottavacolonna[i7] == ottavacolonna[checki7]:
					flag7 = 1
			
			if i8 != checki8:
				if nonacolonna[i8] == nonacolonna[checki8]:
					flag8 = 1
				##########################################################
			if q1 != checkq1:
				if primoquadrato[q1] == primoquadrato[checkq1]:
					flagquadrato1 = 1
					
			if q2 != checkq2:
				if secondoquadrato[q2] == secondoquadrato[checkq2]:
					flagquadrato2 = 1
			
			if q3 != checkq3:
				if terzoquadrato[q3] == terzoquadrato[checkq3]:
					flagquadrato3 = 1
	...
	# printing result
	if(not flag) and not flag1 and not flag2 and not flag3 and not flag4 and not flag5 and not flag6 and not flag7 and not flag8:
	# ~ if (not flagquadrato1) and not flagquadrato2 and not flagquadrato3:
		
			print("colonne",flag,flag1,flag2,flag3,flag4,flag5,flag6,flag7,flag8)
			print("quadrati",flagquadrato1,flagquadrato2,flagquadrato3)
			# ~ print("flag",flag)
			# ~ print()
			# ~ print ("List contains all unique elements")
			print(anagramma)
