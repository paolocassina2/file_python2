...
#ORA SI PUò SCEGLIERE LA CARTELLA DI INSSERENDO IL PATH CON UN INPUT navigando tra le varie caretlle
#ulteriore modifica...> per esempio scegliere ognvi volta il criterio con cui si ordinano i file tra le cartelle(con il nome  della cartella scritto nel file o in base al nome del file)

#ulteriore modifica__>gestire eccezioni (errori) navigazione path in modo che se non esiste la cartella inserita non generi errori e permetta di reinserire il nome della directory
import os
def path():
	...
	# ~ try:
		...
		for xj in lista1.var:#ordina_e poi controlla che i file siano stati spostati corretamente
			...
			if xj[1]!="":
				...
				print(xj[0], "non e stato spostato")
	# ~ except:
		# ~ continue	#file che probabilmente riesce a leggere ma all interno di essi non c e scritto il nome di una cartella (oppure c e scritto CARTELLA=="", quindi non sa dove spostarli
else:
		pass
