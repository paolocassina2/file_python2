...
#cosa NON funziona:
#fare in modo che capisca quale sia la cartella di destinazione anche se il nome della scritta non è scritto nell'ultima riga del file
#SEMBREA CHE NON SPOSTA UN FILE CHE CONTIENE SOLO CARTELLA ciao nonostante questa cartella esista
...
# ~ print(j)
							...
							print("n",NOMECARTELLA1.var,"len",len(NOMECARTELLA1.var))
				# ~ except:
						# ~ continue
# ~ def ciao():					# ~ print("aaaaaaaaaaaaaa",NOMECARTELLA1.var,len(NOMECARTELLA1.var))
					
				# ~ cartelladestination=pathfile+NOMECARTELLA1.var
												
				# ~ shutil.rename(source, destination)
					# ~ else:
								# ~ print("la cartella ",NOMECARTELLA1.var," non esiste")
					
					# ~ else:
						# ~ print("nome cartella contenuto in ",x ," non esiste") 	
				except:
					print("ecc",x)
					continue# ~ def ciao():
				...
				if NOMECARTELLA1.var in listafolders.var:
						...
						jk.var=list(zip(f2.var,destinazione111.var))					
						# ~ print(jk.var)
	...
	# ~ print("ciao",os.listdir(path.var))	
		for xk,xj in jk.var:
			...
			if xk in os.listdir(xj)	:
				print(xk, "è stato spostato correttamente in ",xj)
			elif xk in os.listdir(pathfile):
			# ~ if xk in os.listdir(xj)	:
				print(xk, "non e stato spostato correttamente")
			print()
