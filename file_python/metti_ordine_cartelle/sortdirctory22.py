...
#parta che conta i file sembra che funziona
#risolti problemi relativi navigazioni desktop: se mi trovo in desktop e inserisco desktop non torna indietro nel path
#il programma ignora gli input contenti le barre
################################################################
################################################################
#CASI DA RIVEDERE:
#SE SPOSTO UN GRAN NUMERO DI FILE MAGARI CONTROLLA TROPPO PRESTO CHE SIANO STATI SPOSTATI .PRIMA CHE IL PROGRAMMA FACCIA IN TEMPO A SPOSTARLI TUTTI
#facilitare e velocizzare input: per esempio se inserisco alcuni caratteri e poi tab il programma dovrebbe suggerire quale cartella intendo(per non scrivere tutte le volte il nome intero della cartella)
...
def naviga_tra_i_file():
	...
	def ordina():
		...
		# ~ listafile_leggibili()
		# ~ print("il programma legge tutte le righe all'interno del file e sceglie la cartella  in base all'ultima riga con scritto ###CARTELLA==")
		...
		#COSI FUNZIONA!!!!!!!!PRIMA LEGGO TUTTI I FILE DI UNA CARTELLA E CREO UNA LISTA di tipo zip CHE CONTIENE FILE leggibili E nomi CARTELLE di destinazione DEI FILE LEGGIBILI(nomi cartelle estratte dai file)
				...
				# ~ if "sortdir" not in x:	#per evitare che il programma sposti i programmi come questo che ordinano i file
					# ~ print(x)	
					# ~ try:	
						#solleva eccezioni per esempio se non riesce a leggere il file
		...
		# ~ def ciao():
						if NOMECARTELLA1.var in listafolders.var:
								...
								file_non_spostati.append(x)#se non c'è il nome della cartella scritto nel file o il nome della cartella non corrisponde a una cartella esistente in quel path
							# ~ print(x,"ecc2")
							# ~ color = fg('red')
						...
						file_non_spostati.append(x)#caso in cui il file non contiene il nome di una cartella
						# ~ lista=[]
				# ~ except:
					# ~ if x.endswith('.py') or x.endswith('.txt'):#considero solo i file di testo o .py
						# ~ file_non_leggibili.append(x)
						# ~ print("xx2,",x)
						# ~ continue
			...
			# ~ print("ciao",os.listdir(path.var))	
				for xk,xj in lista:
					...
					if xk in os.listdir(xj)	:
						...
						# ~ conta1=conta1+1
					elif xk in os.listdir(pathfile):		#caso in cui il nome della cartella esiste ma  a causa di una anomalia il programma non viene spostato 
					# ~ if xk in os.listdir(xj)	:
						color = fg('red')
						print(color+str(xk)+ " NON e stato spostato")
						conta1.var=conta1.var+1
					print()
		for xj in file_non_spostati:
			color = fg('red')
			print(color+str(xj)+ " NON e stato spostato")
		from colored import fg
		print("conta")
		color = fg('white')
		print(color +str(conta.var)  +" files su "+ str(conta_file_leggibili)+" sono stati spostati")	
	# ~ print(color +str(conta.var)  +" files su "+ str(conta.var+conta1.var)+" sono stati spostati correttamente")
		color2 = fg('red')
		
		print("numero file NON spostati ",color2+str(len(file_non_spostati)))
		if len(file_non_spostati)>0:
			print("file non spostati ",file_non_spostati)
			print("i file non contengono il nome della cartella di destinazione oppure quest'ultima non esiste")
		# ~ print("file_non_leggibili ",file_non_leggibili)
...
# ~ def hello():		
			...
			# ~ print(path.var)
			if inp!="cd.." and inp!="break" and inp!="" and inp!="ordina"  and inp not in pathlista and inp!="C:/" and inp!="c:/" and "/" not in inp and "\\" not in inp:		
